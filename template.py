...
def call_openai_mini(prompt, temperature=0.7, top_p=0.9, max_tokens=256):
...
def chat_with_system_prompt(system_prompt, user_prompt, model=OPENAI_MODEL, temperature=0.7, top_p=0.9, max_tokens=256):
    from openai import OpenAI
    import os, time
...
def estimate_cost(prompt, response, model=OPENAI_MODEL):
    ...
    pricing = PRICING_PER_1K_TOKENS.get(model, {"input": 0.005, "output": 0.015})
    input_cost = (input_tokens / 1000) * pricing["input"]
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    header = f"{'Prompt':<40} | {'GPT-4o':<40} | {'Mini':<40} | {'GPT-4o Latency':<15} | {'Mini Latency':<15}"
    separator = "-" * len(header)
    lines = [header, separator]
    for result in results:
        lines.append(
            f"{_short(result.get('prompt', '')):<40} | {_short(result.get('gpt4o_response', '')):<40} | {_short(result.get('mini_response', '')):<40} | {result.get('gpt4o_latency', 0):<15.3f} | {result.get('mini_latency', 0):<15.3f}"
        )
    return "\n".join(lines)
